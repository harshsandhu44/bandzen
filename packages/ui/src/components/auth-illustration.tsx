...
/**
 * The picture beside the sign-in and sign-up forms: unDraw's "Studying"
 * (https://undraw.co/illustrations, free for commercial use, no attribution
 * required). A student writing at a desk by a tall window.
 *
 * Inlined rather than served as a file so its colours can be tokens. unDraw's
 * purple accent is `--primary`, which also gives the CMS its plum. The light
 * greys and whites are foreground and background tints, so dark mode doesn't
 * glare. The dark greys, skin and hair keep the illustrator's colours.
 *
 * Decoration only, hence `aria-hidden`.
 */
